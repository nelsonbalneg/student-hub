...
namespace StudentHub\EvaluationBuilder\Services;

use Illuminate\Database\Eloquent\Model;

class TemplatePayloadService
{
    public function build(Model $template): array
    {
        $template->load([
            ...
            'scaleSets' => fn ($query) => $query
                ->where('status', 'active')
                ->with(['ratingScales' => fn ($scaleQuery) => $scaleQuery
                    ->where('status', 'active')
                    ->orderBy('sort_order')
                    ->orderBy('id')]),
        ]);
        ...
        $categories = $template->categories->map(function ($category) use ($template, $scaleSets, $defaultScaleSet): array {
            return [
                ...
                'statements' => $template->statements
                    ...
                    ->map(fn ($statement): array => $this->statement(
                        $statement,
    ...
    private function statement(Model $statement, ?Model $scaleSet): array
    {
        $usesRatingScale = in_array($statement->statement_type, ['likert', 'rating_scale'], true);
        ...
        return [
            ...
            'choices' => $statement->choices->sortBy(['sort_order', 'id'])->map(fn ($choice): array => [
                'id' => $choice->id,
