...
<template>
    <Dialog :open="Boolean(requiredTerms)">
        ...
        >
            <div
                class="border-b border-slate-200 bg-slate-50 px-5 py-4 dark:border-white/10 dark:bg-slate-900"
            >
                <DialogHeader class="space-y-2">
                    ...
                    >
                        <span v-if="requiredTerms?.version"
                            >Version {{ requiredTerms.version }}</span
                        >
                        <span
                            v-if="
                                requiredTerms?.version &&
                                requiredTerms?.published_at_human
                            "
                        >
                            ·
                        </span>
                        <span v-if="requiredTerms?.published_at_human"
                            >Published
                            {{ requiredTerms.published_at_human }}</span
                        >
                    </DialogDescription>
                </DialogHeader>
            </div>
            ...
            <div class="max-h-[56vh] overflow-y-auto px-5 py-5">
                ...
                <div
                    v-else
                    class="grid min-h-44 place-items-center text-sm text-slate-500"
                >
            ...
            <DialogFooter
                class="border-t border-slate-200 bg-white px-5 py-4 dark:border-white/10 dark:bg-slate-950"
            >
