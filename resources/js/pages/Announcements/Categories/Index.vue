        ...
        <section class="border-b border-slate-200 pb-5 dark:border-white/10">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div class="min-w-0">
                    ...
                    <h1 class="mt-3 text-2xl font-bold tracking-normal text-slate-950 dark:text-white">Announcement Categories</h1>
                    <p class="mt-1 max-w-2xl text-sm font-medium text-slate-500 dark:text-slate-400">
                        Organize your announcements into meaningful categories with distinct colors and descriptors.
                    </p>
                </div>
        ...
        <section class="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-slate-950">
            ...
            <div class="flex flex-col gap-1 border-b border-slate-200 px-4 py-3 dark:border-white/10">
                <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                    <h2 class="text-sm font-bold text-slate-950 dark:text-white">Active Categories</h2>
                ...
                <table class="w-full text-sm">
                    <thead class="bg-slate-50 text-left text-[11px] font-bold text-slate-500 uppercase dark:bg-white/5 dark:text-slate-400">
                        <tr>
                            ...
                            <th class="px-4 py-3 w-[400px]">Description</th>
                            <th class="px-4 py-3 text-right">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 dark:divide-white/10">
                        <tr v-for="category in categories.data" :key="category.id" class="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors group">
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-3">
                                    <div class="h-3 w-3 rounded-full border border-black/5 dark:border-white/10" :style="{ backgroundColor: category.color || '#3b82f6' }"></div>
                            ...
                            <td class="px-4 py-3 text-xs font-medium text-slate-500 dark:text-slate-400 truncate">
                        ...
                        <tr v-if="categories.data.length === 0">
                            <td colspan="5" class="p-12 text-center">
                                <div class="flex flex-col items-center justify-center gap-3 opacity-50">
                                    ...
                                    <p class="text-xs font-medium text-slate-500 dark:text-slate-400">Add your first category to start organizing announcements.</p>
                                </div>
                            </td>
                        </tr>
                    </tbody>
    ...
    <div v-if="modal && (modal.type === 'create' || modal.type === 'edit')" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
        <div class="w-full max-w-md rounded-xl border bg-card p-6 shadow-2xl dark:bg-slate-900 animate-in fade-in zoom-in duration-200">
            <div class="mb-6 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <div class="h-10 w-10 rounded-full bg-sky-100 dark:bg-sky-500/10 flex items-center justify-center text-sky-600">
                        <FolderTree class="size-5" />
                    </div>
                    <div>
                        <h2 class="text-lg font-bold text-slate-900 dark:text-white">{{ modal.type === 'create' ? 'Add Category' : 'Edit Category' }}</h2>
                        <p class="text-xs text-slate-500">Classification for announcements.</p>
                    </div>
                </div>
            ...
            <form @submit.prevent="submit" class="space-y-5">
                <div class="space-y-2">
                    <label class="text-xs font-bold text-slate-600 uppercase tracking-wider dark:text-slate-400">Category Name</label>
                    <input 
                        v-model="form.name" 
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        <label class="text-xs font-bold text-slate-600 uppercase tracking-wider dark:text-slate-400">Theme Color</label>
                        <div class="flex gap-2">
                            <input v-model="form.color" type="color" class="h-10 w-12 cursor-pointer rounded border border-slate-200 p-1 dark:border-white/10 dark:bg-slate-950" />
                            <input v-model="form.color" class="flex h-10 w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-xs font-mono text-slate-900 focus:border-sky-400 focus:outline-none dark:border-white/10 dark:bg-slate-950 dark:text-slate-100" />
                        </div>
                    ...
                    <div class="flex items-end pb-2">
                        <label class="flex items-center gap-2 cursor-pointer group">
                            <div class="relative flex items-center">
                                <input type="checkbox" v-model="form.is_active" class="peer h-5 w-5 cursor-pointer appearance-none rounded border border-slate-200 bg-white transition-all checked:bg-sky-600 dark:border-white/10 dark:bg-slate-950" />
                            ...
                            <span class="text-sm font-bold text-slate-700 dark:text-slate-300 group-hover:text-sky-600 transition-colors">Active Status</span>
                        </label>
                    </div>
                </div>

                <div class="space-y-2">
                    <label class="text-xs font-bold text-slate-600 uppercase tracking-wider dark:text-slate-400">Description</label>
                    <textarea 
                ...
                <div class="flex justify-end gap-3 pt-2">
                    <Button type="button" variant="ghost" @click="modal = null" class="font-bold">Cancel</Button>
                    <Button type="submit" :disabled="form.processing" class="font-bold shadow-sky-200 dark:shadow-none">
    ...
    <div v-if="modal && modal.type === 'delete'" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
        <div class="w-full max-w-md rounded-xl border bg-card p-6 shadow-2xl dark:bg-slate-900 animate-in fade-in zoom-in duration-200">
            ...
            <p class="text-sm text-muted-foreground mb-6">
                Are you sure you want to delete the category <strong>{{ modal.category?.name }}</strong>? This action cannot be undone and may affect announcements currently using this category.
            </p>
            <div class="flex justify-end gap-3">
                <Button variant="ghost" @click="modal = null" class="font-bold text-slate-600">Cancel</Button>
                <Button variant="destructive" @click="deleteCategory" class="font-bold">Delete permanently</Button>
            </div>
        </div>
    </div>
</template>
