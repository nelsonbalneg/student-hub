...
const apply = (updateId: number) => {
    router.post(
        `/student-services/clearance/updates/${updateId}/apply`,
        {},
        {
            preserveScroll: true,
            preserveState: true,
    ...
    >
        ...
        >
            <div class="grid gap-0.5">
                <h1
                    class="text-base font-black tracking-widest text-slate-900 uppercase dark:text-slate-100"
                >
                    Clearance Portal
                </h1>
                <p
                    class="text-[10px] font-bold tracking-normal text-slate-400 uppercase dark:text-slate-500"
                >
            ...
            >
                <div
                    class="flex h-8 w-8 items-center justify-center rounded-lg bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-300"
                >
                    <Calendar class="h-4 w-4" />
                </div>
                <div class="grid gap-0">
                    <span
                        class="text-[8px] leading-none font-black tracking-widest text-slate-400 uppercase dark:text-slate-500"
                        >Active Session</span
                    >
                    <span
                        class="text-[11px] font-black tracking-normal text-slate-700 uppercase dark:text-slate-200"
                        >{{ activeSemester.academic_year }} •
                        {{ activeSemester.term }}</span
                    >
                </div>
            </div>
        </div>
        ...
        <div v-if="activeSemester" class="space-y-6">
            ...
            >
                <table class="w-full text-left">
                    <thead>
                        <tr
                            class="border-b border-slate-100 bg-slate-50/50 text-[9px] font-black tracking-[0.2em] text-slate-400 uppercase dark:border-white/10 dark:bg-white/5 dark:text-slate-500"
                        >
                        ...
                        >
                            ...
                            >
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="flex h-9 w-9 items-center justify-center rounded-xl bg-slate-100 text-slate-400 shadow-sm transition-all group-hover:bg-indigo-600 group-hover:text-white dark:bg-white/5"
                                        >
                                            <FileText class="h-4 w-4" />
                                        </div>
                                        <div class="grid gap-0">
                                            <p
                                                class="text-xs font-black tracking-normal text-slate-900 dark:text-slate-100"
                                            >
                                                {{ update.title }}
                                            </p>
                                            <p
                                                class="font-mono text-[9px] font-bold tracking-widest text-indigo-500 uppercase dark:text-indigo-300"
                                            >
                                                SEC-{{
                                                    update.id
                                                        .toString()
                                ...
                                <td
                                    class="px-6 py-4 text-[10px] font-bold text-slate-500 dark:text-slate-400"
                                >
                                    <p class="line-clamp-1 max-w-xs">
                                        {{
                                            update.description ||
                                    ...
                                    <p
                                        class="mt-0.5 text-[9px] tracking-normal text-slate-400 uppercase italic dark:text-slate-500"
                                    >
                                        Deadline:
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    >
                                        ...
                                        >
                                            <span
                                                :class="[
                                                    'rounded-full border px-2.5 py-0.5 text-[8px] font-black tracking-widest uppercase',
                                                    statusColor(
                                                        myClearances.find(
                                                            (c) =>
                                                                c
                                            ...
                                            <button
                                                class="text-[8px] font-black tracking-normal text-slate-400 uppercase hover:text-indigo-600 dark:text-slate-500 dark:hover:text-indigo-300"
                                                @click="
                                                    toggleOffices(update.id)
                                                "
                                            >
                                                {{
                                        ...
                                        <Link
                                            :href="`/student-services/clearance/my-clearance/${myClearances.find((c) => c.clearance_update.id === update.id).id}`"
                                            class="flex h-8 items-center justify-center rounded-lg bg-slate-900 px-4 text-[10px] font-black text-white shadow-lg shadow-slate-900/10 transition-all hover:bg-indigo-600 dark:bg-white/10 dark:hover:bg-indigo-600"
                                        >
                                            Manage
                                        </Link>
                                    </div>
                                    <div v-else class="flex justify-end">
                                        <Button
                                            class="h-8 gap-2 rounded-lg bg-emerald-600 px-5 text-[10px] font-black text-white shadow-lg shadow-emerald-600/10 hover:bg-emerald-700"
                            ...
                            <tr v-if="expandedUpdates.includes(update.id)">
                                ...
                                >
                                    ...
                                    >
                                        <div
                                            v-for="off in update.offices"
                                            :key="off.id"
                                            class="group/node flex items-center justify-between rounded-xl border border-slate-200 bg-white p-2 transition-all hover:border-indigo-300 hover:shadow-md dark:border-white/10 dark:bg-slate-950 dark:hover:border-indigo-400/50"
                                        >
                                            <span
                                                class="truncate text-[9px] font-black tracking-normal text-slate-500 uppercase dark:text-slate-300"
                                            ...
                                            >
                                                ...
                                                >
                                                    <p
                                                        class="mb-2 border-b border-white/5 pb-2 font-black tracking-widest text-red-400 uppercase"
                                                    >
            ...
            <div class="grid gap-4 md:hidden">
                ...
                >
                    <div class="mb-4 flex items-start justify-between gap-4">
                        <div class="grid gap-0">
                            <h3
                                class="text-sm font-black tracking-normal text-slate-900 dark:text-slate-100"
                            >
                                {{ update.title }}
                            </h3>
                            <p
                                class="font-mono text-[9px] font-bold text-indigo-500 uppercase dark:text-indigo-300"
                            >
                                {{ formatDate(update.end_date) }}
                            </p>
                        </div>
                        <div
                            v-if="
                                myClearances.find(
                                    (c) => c.clearance_update.id === update.id,
                                )
                            "
                        >
                            <span
                    ...
                    >
                        ...
                        >
                            <button
                                class="text-[9px] font-black tracking-widest text-slate-400 uppercase hover:text-indigo-600 dark:text-slate-500 dark:hover:text-indigo-300"
                        ...
                        <div
                            v-if="expandedUpdates.includes(update.id)"
                        ...
                        >
                            ...
                            >
                                <span
                                    class="truncate text-[9px] font-black tracking-normal text-slate-600 uppercase dark:text-slate-300"
                                    >{{ off.office.name }}</span
                                >
                                <CheckCircle2
                                    v-if="
                    ...
                    <Button
                        v-else
                        class="h-9 w-full gap-2 bg-emerald-600 text-[10px] font-black text-white uppercase shadow-lg shadow-emerald-600/10 hover:bg-emerald-700"
                        @click="apply(update.id)"
                    >
                        Initiate Application
