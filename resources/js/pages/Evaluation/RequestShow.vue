...
const saveFeedback = () => {
    feedbackForm.post(
        `/admin/evaluations/requests/${props.request.id}/feedback`,
        {
            preserveScroll: true,
            onSuccess: () => {
        ...
        >
            ...
            >
                <div class="flex items-start gap-3">
                    <Link
                        href="/admin/evaluations?tab=requests"
                        class="inline-flex size-9 shrink-0 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-500 transition hover:text-slate-900 dark:border-white/10 dark:bg-white/5 dark:text-slate-300"
                    >
                    ...
                    <div>
                        ...
                        >
                            {{ request.student?.name ?? 'Student Request' }}
                        </h1>
                ...
                <div class="flex flex-wrap gap-2">
                    <span
                        :class="[
                            'inline-flex items-center rounded-md border px-3 py-2 text-xs font-bold uppercase',
                            statusStyles[request.status],
                        ]"
                        >{{ request.status.replace('_', ' ') }}</span
                    >
                    <Button
                        v-if="can.evaluate && request.status === 'submitted'"
                        :disabled="statusForm.processing"
                        @click="updateStatus('under_evaluation')"
                    ...
                    <Button
                        ...
                        variant="outline"
                        @click="openFeedback()"
                        ><MessageSquare class="mr-2 size-4" />Add
                        Feedback</Button
                    >
                    <Button
                        v-if="can.feedback"
                        ...
                        @click="openFeedback('needs_action')"
                        >Needs Action</Button
                    >
                    <Button
                        v-if="can.feedback"
                        ...
                        @click="openFeedback('resolved')"
                        >Resolved</Button
                    >
                    <Button
                        v-if="can.markDone && request.status !== 'done'"
                        :disabled="statusForm.processing"
                        @click="updateStatus('done')"
                        ><CheckCircle2 class="mr-2 size-4" />Mark Done</Button
                    >
                </div>
            </div>
        </section>
        ...
        <main class="grid grid-cols-1 gap-5 xl:grid-cols-12">
            <section class="space-y-5 xl:col-span-8">
                ...
                >
                    ...
                    >
                        <p
                            class="text-sm leading-6 font-medium text-slate-800 dark:text-slate-100"
                        >
                            {{ request.intent }}
                        </p>
                        <p
                            v-if="request.remarks"
                            class="mt-3 border-t border-slate-200 pt-3 text-sm text-slate-500 dark:border-white/10"
                        >
                            {{ request.remarks }}
                        </p>
                    </div>
                ...
                >
                    ...
                    <div class="mt-4 space-y-3">
                        <div
                            v-for="feedback in request.feedbacks"
                            :key="feedback.id"
                        ...
                        >
                            ...
                            >
                                ...
                                {{ feedback.visibility.replace('_', ' ') }} ·
                                {{ formatDate(feedback.created_at) }}
                            </p>
                        </div>
                        <p
                            v-if="request.feedbacks.length === 0"
            ...
            <aside class="space-y-5 xl:col-span-4">
                ...
                >
                    ...
                    <div class="mt-4 space-y-3 text-sm">
                        ...
                        <p class="text-slate-500">
                            {{ formatDate(request.period?.start_date ?? null) }}
                            to
                            {{ formatDate(request.period?.end_date ?? null) }}
                        </p>
                    </div>
                </div>
    ...
    >
        ...
        >
            ...
            <div class="mt-5 flex justify-end gap-2">
                <Button variant="outline" @click="feedbackOpen = false"
                    >Cancel</Button
                >
                <Button
