...
const canSubmit = computed(
    () => props.can.submitIntent && props.activePeriod && !props.currentRequest,
);
...
const cancelRequest = () => {
    ...
    actionForm.patch(
        `/student/evaluation/requests/${cancelTarget.value.id}/cancel`,
        {
            preserveScroll: true,
            ...
            <div class="xl:col-span-7">
                ...
                >
                    ...
                    >
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                >
                                    Visible only during active registrar calls
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-if="activePeriod" class="p-5">
                        ...
                        >
                            <div>
                                <div class="flex flex-wrap items-center gap-2">
                                    <span
                                        class="inline-flex rounded-md border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-[10px] font-bold text-emerald-700 uppercase dark:border-emerald-400/30 dark:bg-emerald-500/10 dark:text-emerald-200"
                                    >
                                        {{ activePeriod.status }}
                                    </span>
                                    <span
                                        class="inline-flex rounded-md border border-slate-200 bg-slate-50 px-2.5 py-1 text-[10px] font-bold text-slate-600 uppercase dark:border-white/10 dark:bg-white/5 dark:text-slate-300"
                                    >
                                ...
                                <h3
                                    class="mt-3 text-xl font-bold text-slate-950 dark:text-white"
                                >
                                    {{ activePeriod.title }}
                                </h3>
                                <p
                                ...
                                <div
                                    class="mt-4 flex flex-wrap gap-3 text-xs font-bold text-slate-500"
                                >
                                    <span
                            ...
                            <Button
                                :disabled="!canSubmit"
                                @click="intentOpen = true"
                                class="h-10 rounded-md px-5 font-bold"
                            >
                        ...
                        <div
                            v-if="currentRequest"
                            class="mt-5 rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/5"
                        >
                            <div
                            ...
                            >
                                ...
                                <span
                                    :class="[
                                        'inline-flex w-fit rounded-md border px-2.5 py-1 text-[10px] font-bold uppercase',
                                        statusStyles[currentRequest.status],
                                    ]"
                                >
                                    {{
                            ...
                            <div
                                v-if="currentRequest.registrar_feedback"
                                class="mt-3 rounded-md border border-sky-100 bg-white p-3 text-sm font-medium text-slate-700 dark:border-sky-400/20 dark:bg-slate-900 dark:text-slate-200"
                            >
                    ...
                    <div v-else class="p-10 text-center">
                        ...
                        >
                            Please check back once the Registrar opens an
                            evaluation call.
                        </p>
                    </div>
                </div>
                ...
                >
                    ...
                    <div class="mt-5 space-y-5">
                        <div
                            ...
                            :key="item"
                            class="relative pl-7 before:absolute before:top-5 before:left-[7px] before:h-8 before:w-[2px] before:bg-slate-100 last:before:hidden dark:before:bg-white/10"
                        >
                            <div
                                class="absolute top-1 left-0 size-3.5 rounded-full border-4 border-white bg-sky-500 dark:border-slate-950"
                            ></div>
                            <p
            ...
            >
                <div>
                    ...
                    <p
                        class="text-xs font-medium text-slate-500 dark:text-slate-400"
                    >
                        Your submitted evaluation intents
                    </p>
                </div>
            </div>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <Button
                                        ...
                                        variant="outline"
                                        size="sm"
                                        class="h-8 text-xs font-bold text-red-600"
            ...
            >
                ...
                <div class="flex gap-1">
                    <button
                        ...
                        :key="link.label"
                        :disabled="!link.url"
                        class="rounded-md px-3 py-1 text-xs font-bold disabled:opacity-40"
                        :class="
                            link.active
        ...
        >
            <div class="flex items-center justify-between">
                <h2 class="text-lg font-bold text-slate-900 dark:text-white">
                    Submit Intent for Evaluation
                </h2>
                <button @click="intentOpen = false">
            ...
            <div class="mt-4 space-y-4">
                <label class="grid gap-2">
                    ...
                    <textarea
                        v-model="form.intent"
                        class="min-h-28 rounded-md border border-slate-200 bg-white p-3 text-sm dark:border-white/10 dark:bg-slate-900 dark:text-white"
                        placeholder="Describe your intent for evaluation"
                    ></textarea>
                ...
                <label class="grid gap-2">
                    ...
                    <textarea
                        ...
                        class="min-h-20 rounded-md border border-slate-200 bg-white p-3 text-sm dark:border-white/10 dark:bg-slate-900 dark:text-white"
            ...
            <div class="mt-5 flex justify-end gap-2">
                <Button variant="outline" @click="intentOpen = false"
                    >Cancel</Button
                >
    ...
    >
        ...
        >
            ...
            <p class="mt-1 text-sm text-slate-500">
                This can only be done while the request is still submitted.
            </p>
            <div class="mt-5 flex justify-end gap-2">
    ...
    >
        ...
        >
            <div class="flex items-center justify-between">
                ...
                <button @click="detailsTarget = null">
                    <X class="size-5 text-slate-400" />
                </button>
            </div>
            <div class="mt-5 space-y-4">
                ...
                >
                    ...
                    <p
                        class="mt-1 text-sm font-medium text-slate-800 dark:text-slate-100"
                    >
                        {{ detailsTarget.intent }}
                    </p>
                </div>
                <div
                    class="rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/5"
                >
                    ...
                    <div
                        v-if="detailsTarget.feedbacks.length"
                        class="mt-3 space-y-2"
                    >
                        <div
                            v-for="feedback in detailsTarget.feedbacks"
                            :key="feedback.id"
                            class="rounded-md bg-white p-3 dark:bg-slate-900"
                        >
                            <p
                            ...
                            <p
                                class="mt-1 text-[10px] font-bold text-slate-400 uppercase"
                            >
                                {{ feedback.author?.name ?? 'Registrar' }} ·
