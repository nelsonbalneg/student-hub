...
const activeCategoryName = computed(() => {
    ...
    return (
        props.categories.data.find((category) => category.slug === activeCategory.value)
            ?.name || 'Selected Topic'
    );
});

const articleCount = computed(() => props.faqs.meta?.total ?? props.faqs.data.length);
...
<template>
    ...
    <div class="flex min-h-screen flex-col bg-slate-50/70 dark:bg-slate-950">
            ...
            <div class="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
                <div class="flex items-start gap-4">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="mt-1 max-w-3xl text-sm leading-6 text-slate-600 dark:text-slate-400">
                            Find official answers for enrollment, grades, accounts, academic services, and StudentHub workflows.
                        </p>
                    </div>
                </div>
                ...
                <div class="grid gap-3 sm:grid-cols-2 xl:min-w-[420px]">
                    <div class="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 dark:border-white/10 dark:bg-white/5">
                        ...
                        <p class="mt-1 text-sm font-bold text-slate-950 dark:text-white">
        ...
        <main class="grid w-full flex-1 gap-5 p-4 lg:grid-cols-[280px_minmax(0,1fr)] lg:p-8">
            <aside class="lg:sticky lg:top-24 lg:self-start">
                <div class="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-slate-900/60">
                ...
                <div class="mt-5 rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-slate-900/60">
                    <div class="flex items-center gap-3">
                        <div class="flex size-9 items-center justify-center rounded-lg bg-blue-50 text-blue-700 dark:bg-blue-500/10 dark:text-blue-300">
            ...
            <section class="space-y-5">
                <div class="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-slate-900/60">
                    <div class="border-b border-slate-200 p-4 dark:border-white/10">
                        ...
                        <input
                            ...
                            type="text"
                                ...
                                class="h-11 w-full rounded-lg border border-slate-200 bg-slate-50 pl-11 pr-11 text-sm font-medium text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-emerald-300 focus:bg-white focus:ring-4 focus:ring-emerald-500/10 dark:border-white/10 dark:bg-white/5 dark:text-white dark:focus:border-emerald-500/40 dark:focus:bg-slate-950"
                        />
                            <button
                                v-if="search"
                                type="button"
                                @click="clearSearch"
                                class="absolute right-3 top-1/2 flex size-7 -translate-y-1/2 items-center justify-center rounded-md text-slate-400 transition hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-white/10 dark:hover:text-white"
                            >
                                <X class="size-4" />
                        </button>
                    </div>
                    ...
                    <div class="flex flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                        <div class="flex min-w-0 items-center gap-3">
                            <div class="flex size-9 shrink-0 items-center justify-center rounded-lg bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300">
                                <BookOpen class="size-4" />
                        </div>
                            <div class="min-w-0">
                                <h2 class="truncate text-base font-bold text-slate-950 dark:text-white">
                                    {{ search ? 'Search Results' : activeCategoryName }}
                                </h2>
                        ...
                        <button
                            ...
                            type="button"
                            @click="selectCategory(null); clearSearch()"
                ...
                <section
                    v-if="!activeCategory && !search && featuredFaqs.data.length > 0"
                    ...
                    <div class="divide-y divide-slate-200 dark:divide-white/10">
                        ...
                        >
                            <div class="min-w-0">
                                ...
                                <h3 class="line-clamp-1 text-sm font-bold text-slate-950 transition group-hover:text-emerald-700 dark:text-white dark:group-hover:text-emerald-300">
                                    {{ faq.question }}
                                </h3>
                            ...
                            <ArrowRight class="mt-1 size-4 shrink-0 text-slate-400 transition group-hover:text-emerald-600" />
                        </Link>
                    </div>
                </section>
                ...
                <section class="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-slate-900/60">
                    <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4 dark:border-white/10">
                        <div class="flex items-center gap-3">
                            <div class="flex size-9 items-center justify-center rounded-lg bg-slate-100 text-slate-700 dark:bg-white/10 dark:text-slate-200">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-slate-500 dark:text-slate-400">
                                    Select an article to view the complete answer.
                                </p>
                            </div>
                        </div>
                        <p class="hidden text-xs font-bold uppercase tracking-wide text-slate-400 sm:block">
                            {{ faqs.data.length }} shown
                        </p>
                    </div>

                    <div v-if="faqs.data.length > 0" class="divide-y divide-slate-200 dark:divide-white/10">
                        <Link
                        ...
                        >
                            <div class="min-w-0">
                                <div class="mb-2 flex items-center gap-3">
                                    <span
                                        ...
                                        :style="{ backgroundColor: faq.category.color }"
                                    ></span>
                                    <span class="text-xs font-bold uppercase tracking-wide text-slate-400 transition group-hover:text-emerald-600 dark:text-slate-500">
                                        {{ faq.category.name }}
                                    </span>
                                    </div>
                                <h3 class="line-clamp-1 text-sm font-bold text-slate-950 transition group-hover:text-emerald-700 dark:text-white dark:group-hover:text-emerald-300">
                            ...
                            <div class="flex size-8 shrink-0 items-center justify-center rounded-lg text-slate-400 transition group-hover:bg-emerald-50 group-hover:text-emerald-600 dark:group-hover:bg-emerald-500/10 dark:group-hover:text-emerald-300">
                    ...
                    >
                        <div class="mb-5 flex size-14 items-center justify-center rounded-xl bg-slate-100 text-slate-400 dark:bg-white/5">
                            <Search class="size-7" />
                        </div>
                        ...
                        <p class="mt-2 max-w-md text-sm leading-6 text-slate-500 dark:text-slate-400">
                            No FAQ articles match the current keyword or topic. Try another term or clear your filters.
                        </p>
                        <Button
                            @click="clearSearch"
                            variant="outline"
