...
const activeFilterCount = computed(
    () =>
        [filters.value.status, props.can.viewTermId ? filters.value.term_id : '']
...
const applyFilters = () => {
    router.get(
        internetAccountRoutes.index.url(),
        {
            ...
            ...(props.can.viewTermId ? { term_id: filters.value.term_id } : {}),
        },
        {
            preserveState: true,
            preserveScroll: true,
...
<template>
    ...
    <div class="flex h-full flex-1 flex-col gap-5 p-4 lg:p-6">
        <section class="border-b border-slate-200 pb-5 dark:border-white/10">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div class="min-w-0">
                    ...
                    <h1 class="mt-3 text-2xl font-bold tracking-normal text-slate-950 dark:text-white">
                        Internet Account
                    </h1>
                    <p class="mt-1 max-w-2xl text-sm font-medium text-slate-500 dark:text-slate-400">
                        Request and manage internet accounts for the
                        current active term.
                    </p>
                </div>

                <Button class="h-10 rounded-md px-4" :disabled="!canSubmit || form.processing" @click="submit">
        ...
        <section class="grid gap-3 md:grid-cols-2"
            :class="can.manage || can.viewTermId ? 'xl:grid-cols-4' : 'xl:grid-cols-3'">
            <div v-if="can.manage"
                class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-slate-950">
                <div class="flex items-center gap-2 text-[11px] font-bold text-slate-500 uppercase dark:text-slate-400">
                    <CalendarClock class="size-4 text-sky-600" />
                    Active Semester
                </div>
                <div class="mt-2 truncate text-lg font-bold text-slate-950 dark:text-white">
        ...
        <section
            ...
            <div class="flex flex-col gap-1 border-b border-slate-200 px-4 py-3 dark:border-white/10">
                ...
                <div v-if="can.manage" class="mt-3 flex flex-col gap-2 lg:flex-row lg:items-center">
                    <div class="relative flex-1">
                        <Search class="absolute top-1/2 left-2.5 size-3.5 -translate-y-1/2 text-slate-400" />
                        <input v-model="search" type="text" placeholder="Search username, student, semester, or term..."
                ...
                <Transition enter-active-class="transition duration-200 ease-out"
                    ...
                    <div v-if="can.manage && filterOpen"
                        ...
                        <label v-if="can.viewTermId"
                            class="grid gap-1 text-xs font-bold text-slate-600 dark:text-slate-300">
                            Term ID
                            <input v-model="filters.term_id" type="text" placeholder="Filter by term ID"
            ...
            <div v-if="requests.data.length === 0"
                ...
                <p class="max-w-md text-xs font-medium text-slate-500 dark:text-slate-400">
                    Your first request will be queued for provisioning.
                </p>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-white/10">
                        <tr v-for="request in requests.data" :key="request.id"
                            ...
                            <td class="px-4 py-3">
                                ...
                                <div v-if="request.failure_reason"
                            ...
                            <td class="px-4 py-3">
                                <div
                                    ...
                                    <button type="button"
                                        ...
                                        title="Show or hide password" :disabled="!request.password"
                                    ...
                                    <button type="button"
                                        class="inline-flex size-7 items-center justify-center rounded-md text-slate-500 transition hover:bg-white hover:text-slate-950 disabled:cursor-not-allowed disabled:opacity-40 dark:text-slate-300 dark:hover:bg-white/10 dark:hover:text-white"
                                        title="Copy password" :disabled="!request.password"
                            ...
                            <td class="px-4 py-3">
                                <span
                                    ...
                                    <component :is="statusIcons[request.status]" class="size-3.5" />
                                    {{ request.status }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-xs font-semibold text-slate-600 dark:text-slate-300">
            ...
            <div v-if="requests.meta.last_page > 1"
                ...
                <div class="flex items-center gap-1">
                    <button
                        ...
                        :disabled="!requests.links[0]?.url" @click="navigatePage(requests.links[0]?.url)">
                        <ChevronsLeft class="size-4" />
                    </button>
                    <button
                        class="inline-flex size-8 items-center justify-center rounded-md text-slate-500 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-40 dark:text-slate-300 dark:hover:bg-white/10"
                        :disabled="requests.meta.current_page === 1" @click="
                            navigatePage(
                    ...
                    <template v-for="link in requests.links.slice(1, -1)" :key="link.label">
                        <button v-if="link.label !== '...'" type="button"
                            class="inline-flex size-8 items-center justify-center rounded-md text-xs font-bold transition"
                            :class="link.active
                                ? 'bg-sky-600 text-white'
                                : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-white/10'
                                " @click="navigatePage(link.url)" v-html="link.label" />
                        <span v-else class="px-1 text-slate-400">...</span>
                    </template>
                    <button
                        class="inline-flex size-8 items-center justify-center rounded-md text-slate-500 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-40 dark:text-slate-300 dark:hover:bg-white/10"
                        :disabled="requests.meta.current_page ===
                            requests.meta.last_page
                            " @click="
                                navigatePage(
                                    requests.links[requests.meta.current_page + 1]
                    ...
                    <button
                        class="inline-flex size-8 items-center justify-center rounded-md text-slate-500 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-40 dark:text-slate-300 dark:hover:bg-white/10"
                        :disabled="!requests.links[requests.links.length - 1]?.url
                            " @click="
                                navigatePage(
