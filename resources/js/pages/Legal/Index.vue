...
const activeDocuments = computed(
    () => props.documents.data.filter((document) => document.is_active).length,
);

const typeLabel = (value: string) =>
    props.types.find((type) => type.value === value)?.label ?? value;
...
<template>
    ...
    <div
        class="flex h-full flex-1 flex-col gap-3 bg-slate-50/60 p-3 lg:p-4 dark:bg-slate-950"
    >
        <header class="border-b border-slate-200 pb-3 dark:border-white/10">
            ...
            >
                <div>
                    <p
                        class="text-xs font-bold tracking-wide text-emerald-600 uppercase dark:text-emerald-300"
                    >
                        Settings
                    </p>
                    <h1
                    ...
                    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                        Manage active Terms, Cookie Policy, and Privacy Policy
                        versions.
                    </p>
                </div>
        ...
        <section class="grid gap-2 sm:grid-cols-3">
            <div
                class="rounded-lg border border-slate-200 bg-white px-3 py-2.5 dark:border-white/10 dark:bg-slate-950"
            >
        ...
        >
            <div
                class="flex flex-col gap-2 border-b border-slate-200 p-3 sm:flex-row sm:items-center sm:justify-between dark:border-white/10"
            >
                ...
                <p class="text-xs font-medium text-slate-500">
                    Page {{ documents.meta.current_page }} of
                    {{ documents.meta.last_page }}
                </p>
            </div>
            ...
            >
                <div>
                    <FileText
                        class="mx-auto mb-2 size-10 text-slate-300 dark:text-slate-700"
                    />
                    <h2
                    ...
                    <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                        Create the first Terms, Cookie Policy, or Privacy Policy
                        document.
                    </p>
                </div>
            </div>
            ...
            <div v-else class="overflow-x-auto">
                ...
                >
                    <thead
                        class="bg-slate-50 text-xs text-slate-500 uppercase dark:bg-white/[0.03]"
                    >
                        <tr>
                    ...
                    >
                        <tr
                            ...
                            :key="document.id"
                            class="hover:bg-slate-50/80 dark:hover:bg-white/[0.03]"
                        >
                            <td class="px-3 py-2">
                                <div
                            ...
                            <td class="px-3 py-2">
                                <span
                                    class="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-bold text-slate-600 dark:bg-white/10 dark:text-slate-300"
                                >
            ...
            <div
                ...
                class="flex items-center justify-end gap-1 border-t border-slate-200 p-2 dark:border-white/10"
            >
                <button
                    class="legal-page-btn"
                    :disabled="!documents.links[0]?.url"
                    @click="navigatePage(documents.links[0]?.url)"
                >
                ...
                <button
                    class="legal-page-btn"
                    :disabled="
                        !documents.links[documents.links.length - 1]?.url
                    "
                    @click="
                        navigatePage(
                            documents.links[documents.links.length - 1]?.url,
    ...
    >
        <div
            class="w-full max-w-md rounded-lg bg-white p-5 shadow-2xl dark:bg-slate-950"
        >
            ...
            <div class="mt-5 flex justify-end gap-2">
                <Button variant="outline" @click="pendingAction = null"
                    >Cancel</Button
                >
                <Button
                    :class="
