...
const runAction = () => {
    ...
    if (confirmAction.value === 'activate') {
        router.patch(
            activate.url(props.document.id),
            {},
            {
                preserveScroll: true,
                preserveState: true,
    ...
    if (confirmAction.value === 'deactivate') {
        router.patch(
            deactivate.url(props.document.id),
            {},
            {
                preserveScroll: true,
                preserveState: true,
...
<template>
    ...
    >
        ...
        <section class="grid gap-4 md:grid-cols-4">
            <div
                class="rounded-lg border border-slate-200 bg-white p-4 dark:border-white/10 dark:bg-slate-950"
            >
                <p class="text-xs font-bold text-slate-400 uppercase">Status</p>
                <p
                    class="mt-2 inline-flex items-center gap-1 text-sm font-bold"
                    :class="
                        document.is_active
                            ? 'text-emerald-600'
                            : 'text-slate-500'
            ...
            >
                ...
                <p
                    class="mt-2 text-sm font-bold text-slate-950 dark:text-white"
                >
        ...
        <article
            class="rounded-lg border border-slate-200 bg-white p-6 dark:border-white/10 dark:bg-slate-950"
        >
            <div
                ...
                {{
                    confirmAction === 'delete'
                        ...
                        : confirmAction === 'activate'
                          ...
                          : 'This document will no longer appear in public legal drawers.'
                }}
            </p>
