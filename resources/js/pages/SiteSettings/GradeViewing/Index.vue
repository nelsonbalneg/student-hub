...
const submitCreate = () => {
    form.post(route('admin.site-settings.grade-viewing.store'), {
        onSuccess: () => {
...
const submitUpdate = () => {
    ...
    form.patch(route('admin.site-settings.grade-viewing.update', selectedRule.value.id), {
        onSuccess: () => {
...
const deleteRule = (rule: Rule) => {
    if (confirm('Are you sure you want to delete this rule? This will be logged.')) {
        router.delete(route('admin.site-settings.grade-viewing.destroy', rule.id));
    }
};

const toggleRule = (rule: Rule) => {
    router.patch(route('admin.site-settings.grade-viewing.toggle', rule.id));
};

const getActionColor = (action: string) => {
    switch (action) {
        case 'created': return 'text-emerald-600 bg-emerald-50 dark:bg-emerald-500/10 dark:text-emerald-400';
        case 'updated': return 'text-blue-600 bg-blue-50 dark:bg-blue-500/10 dark:text-blue-400';
        case 'deleted': return 'text-red-600 bg-red-50 dark:bg-red-500/10 dark:text-red-400';
        case 'toggled': return 'text-amber-600 bg-amber-50 dark:bg-amber-500/10 dark:text-amber-400';
