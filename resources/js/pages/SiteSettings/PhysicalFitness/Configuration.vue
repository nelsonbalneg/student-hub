...
const destroyRecord = (type: string, id: number) => {
    ...
    if (type === 'field') router.delete(fieldRoutes.destroy.url(id), options);
};
...
<template>
    ...
    <SiteSettingsLayout>
        <div
            class="flex min-h-0 w-full min-w-0 flex-1 flex-col text-slate-900 dark:text-slate-100"
        >
            <header
                class="border-b border-slate-100 px-5 py-4 dark:border-white/10"
            >
                <p
                    class="text-[11px] font-bold tracking-[0.2em] text-emerald-600 uppercase dark:text-emerald-300"
                >
                    Site Settings
                </p>
                <div
            ...
            >
                <section class="pft-panel">
                    ...
                    >
                        ...
                        <span
                            class="pft-badge"
                            :class="statusClass(component.is_active)"
                            >{{ component.is_active ? 'On' : 'Off' }}</span
                        >
                    </button>
                    <div v-if="selectedComponent" class="pft-actions">
                        <button
                            v-if="can.update"
                            @click="openComponent(selectedComponent)"
                        >
                ...
                <section class="pft-panel">
                    <div class="pft-panel-head">
                        <h2>Categories</h2>
                        <button
                            v-if="can.create && selectedComponent"
                            class="pft-icon-btn"
                            @click="openCategory()"
                        >
                            <Plus class="size-4" />
                    ...
                    <button
                        v-for="category in selectedComponent?.categories ?? []"
                        :key="category.id"
                        type="button"
                        class="pft-row"
                        :class="
                    ...
                    >
                        ...
                        <span
                            ...
                            >{{ category.is_active ? 'On' : 'Off' }}</span
                        >
                    </button>
                ...
                <section class="pft-panel">
                    <div class="pft-panel-head">
                        <h2>Test Types</h2>
                        <button
                            v-if="can.create && selectedCategory"
                            class="pft-icon-btn"
                            @click="openTestType()"
                        >
                            <Plus class="size-4" />
                    ...
                    <button
                        v-for="testType in selectedCategory?.test_types ?? []"
                        :key="testType.id"
                        type="button"
                        class="pft-row"
                        :class="
                            testType.id === selectedTestTypeId
                                ? 'pft-row-active'
                                : ''
                        "
                    ...
                    >
                        ...
                        <span
                            class="pft-badge"
                            :class="statusClass(testType.is_active)"
                            >{{ testType.is_active ? 'On' : 'Off' }}</span
                        >
                    </button>
                    <div v-if="selectedTestType" class="pft-actions">
                        <button
                            v-if="can.update"
                            @click="openTestType(selectedTestType)"
                        >
                            <Pencil class="size-3.5" /> Edit
                        </button>
                        <button
                ...
                <section class="pft-panel">
                    <div class="pft-panel-head">
                        ...
                        <button
                            v-if="can.create && selectedTestType"
                            class="pft-icon-btn"
                            @click="openField()"
                        >
                            <Plus class="size-4" />
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-left text-sm">
                            ...
                            >
                                <tr
                                    v-for="field in selectedTestType?.configurations ??
                                    []"
                                    :key="field.id"
                                >
                                    <td
                                        class="px-3 py-3 font-semibold text-slate-800 dark:text-slate-100"
                                    >
                                        {{ field.field_label }}
                                    </td>
                                    <td
                                    ...
                                    <td class="px-3 py-3">
                                        <div class="flex justify-end gap-1">
                                            ...
                                            <button
                                                v-if="can.delete"
                                                class="pft-icon-btn text-red-600"
                                                @click="
                                                    destroyRecord(
                                                        'field',
                                                        field.id,
            ...
            >
                ...
                >
                    ...
                    <label class="pft-check"
                        ><input
                            v-model="componentForm.is_active"
                            type="checkbox"
                        />
                        Active</label
                    >
                ...
                <div
                    v-else-if="modal.type === 'category'"
                    class="grid gap-3 md:grid-cols-2"
                >
                    <select
                        v-model="categoryForm.pft_component_id"
                        class="pft-input"
                    >
                        <option
                            v-for="component in components"
                ...
                <div
                    v-else-if="modal.type === 'testType'"
                    class="grid gap-3 md:grid-cols-2"
                >
                    <select
                        v-model="testTypeForm.pft_category_id"
                        class="pft-input"
                    >
                        <option
                            v-for="category in selectedComponent?.categories ??
                    ...
                    />
                    <input
                        v-model="testTypeForm.slug"
                        class="pft-input"
                        placeholder="Slug (optional)"
                    />
                    <input
                        v-model="testTypeForm.unit"
                        class="pft-input"
                        placeholder="Unit"
                    />
                    <input
                        v-model.number="testTypeForm.sort_order"
                        class="pft-input"
                        type="number"
                        placeholder="Sort order"
                    />
                    <label class="pft-check"
                        ><input
                            v-model="testTypeForm.is_active"
                            type="checkbox"
                        />
                        Active</label
                    >
                    <textarea
                        v-model="testTypeForm.description"
                        class="pft-input md:col-span-2"
                        placeholder="Description"
                    ></textarea>
                </div>
                ...
                <div v-else class="grid gap-3 md:grid-cols-2">
                    ...
                    >
                    <label class="pft-check"
                        ><input v-model="fieldForm.is_active" type="checkbox" />
                        Active</label
                    >
                </div>
